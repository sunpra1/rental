package softwarica.sunilprasai.cuid10748110.rental.ui.edit_house_tenancy
...
private const val TAG = "EditTenancyFragment"
private const val REQUEST_ACCESS_EXTERNAL_STORAGE: Int = 4001
private const val REQUEST_CODE_SELECT_PHOTOS: Int = 4002
...
class EditHouseTenancyFragment : Fragment(), View.OnClickListener,
    ...
    private lateinit var mUpdateTenantBtn: MaterialButton
    ...
    private val mRentAccrueSpnItemSelectedListener = object : AdapterView.OnItemSelectedListener {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_edit_house_tenancy, container, false)
        ...
        mUpdateTenantBtn = view.findViewById(R.id.updateTenancyBtn)
        mUpdateTenantBtn.setOnClickListener(this)
        ...
        val mRoomTypeSpnItemsAdapter: ArrayAdapter<String> = object : ArrayAdapter<String>(
            ...
            override fun isEnabled(position: Int): Boolean =
                if (position == 0) false else super.isEnabled(position)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        mViewModel.getHouses().observe(requireActivity()) {
            if (isAdded) {
                if (mParamHousePosition != -1 && mParamTenancyPosition != -1 && mParamHousePosition < it.size && mParamTenancyPosition < it[mParamHousePosition].tenancies!!.size && it[mParamHousePosition].id == mParamHouseID && it[mParamHousePosition].tenancies!![mParamTenancyPosition].id == mParamTenancyID) {
                    val tenancy = it[mParamHousePosition].tenancies!![mParamTenancyPosition]
                    updateView(tenancy)
                } else {
    ...
    private fun updateView(tenancy: Tenancy) {
        determineIsValueProvidedValidPhoneOrEmail(tenancy.tenant!!.phone.toString())
        mSelectedTenancyStartDate = tenancy.startDate
        mTenantPhoneOrEmailEt.setText(tenancy.tenant!!.phone.toString())
        mTenancyStartDateCV.date = tenancy.startDate!!.time
        mRentAmountEt.setText(tenancy.amount.toString())
        mRoomCountEt.setText(tenancy.roomCount.toString())
        mRoomTypeSpn.setSelection(TENANCY_ROOM_TYPE.indexOf(tenancy.roomType.toString()))
        mRentAccrueSpn.setSelection(TENANCY_RENT_ACCRUES_AT.indexOf(tenancy.accrue.toString()))
        mRecyclerViewAdapterOfCurrentImages.setSelectedImages(tenancy.images!!)
    }
    ...
    companion object {
        ...
        fun newInstance(
            ...
            EditHouseTenancyFragment().apply {
                    ...
                    if (!hasFocus) {
                        mTenantPhoneOrEmailEt.text?.let {
                            if (it.trim().isEmpty()) {
                                ...
                                if (!mIsValidPhone && !mIsValidEmail) {
                                    val message: String? =
    ...
    override fun onKey(v: View?, keyCode: Int, event: KeyEvent?): Boolean {
        v?.let {
            if (v.id == mTenantPhoneOrEmailEt.id) {
                if (mTenantPhoneOrEmailEt.text != null && mTenantPhoneOrEmailEt.text!!.isNotEmpty()) {
                    val phoneOrEmail = mTenantPhoneOrEmailEt.text!!.trim().toString()
                    determineIsValueProvidedValidPhoneOrEmail(phoneOrEmail)
                    if (mIsValidPhone || mIsValidEmail)
    ...
    private fun determineIsValueProvidedValidPhoneOrEmail(phoneOrEmail: String) {
        mIsValidPhone =
    ...
    private fun updateTenancy(tenancy: Tenancy) {
        ...
        val consumer = APIService.getService(APIConsumer::class.java)
        val editTenancy = consumer.updateTenancy(
        ...
        editTenancy.enqueue(object : Callback<ResponseBody> {
            override fun onResponse(
                call: Call<ResponseBody>,
                response: Response<ResponseBody>
            ) {
                if (response.isSuccessful && response.body() != null) {
                    ...
                    AppToast.show(
                        mContext,
                        "Tenancy has been updated successfully",
                        Toast.LENGTH_LONG
                    )
                    clearFields()
                    requireActivity().supportFragmentManager.popBackStack()
