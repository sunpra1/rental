...
class ViewTenantTenancyHistoryPaymentFragment : Fragment(), SwipeRefreshLayout.OnRefreshListener {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(
            R.layout.fragment_view_tenant_tenancy_history_payment,
            container,
            false
        )
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        mViewModel.getTenantTenanciesHistory().observe(requireActivity()) {
            if (isAdded) {
                if (it != null && mParamTenancyPosition != -1 && mParamPaymentPosition != -1 && mParamPaymentPosition < it[mParamTenancyPosition].payments!!.size && it[mParamTenancyPosition].id == mParamTenancyID && it[mParamTenancyPosition].payments!![mParamPaymentPosition].id == mParamPaymentID) {
                    mTenancy = mViewModel.getTenantTenancyHistory(mParamTenancyPosition)
                    mPayment = mViewModel.getTenantTenancyPaymentHistory(
    ...
    companion object {
        ...
        fun newInstance(
            ...
            ViewTenantTenancyHistoryPaymentFragment().apply {
    ...
    override fun onRefresh() {
        ...
        val getTenantTenancyHistory =
            consumer.getTenantTenancyHistory(
                UserToken.getInstance(mContext).token!!,
                mParamTenancyID!!
            )
        getTenantTenancyHistory.enqueue(object : Callback<ResponseBody> {
            override fun onResponse(
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    mViewModel.replaceTenantTenancyHistory(
                        mParamTenancyPosition,
                    ...
                    if (errorResponse.has("message")) {
                        ...
                        displayAPIErrorResponseDialog("Unable to refresh your payment history details. Please try again later")
                    }
                } else {
                    displayAPIErrorResponseDialog("Unable to refresh your payment history details. Please try again later")
                }
