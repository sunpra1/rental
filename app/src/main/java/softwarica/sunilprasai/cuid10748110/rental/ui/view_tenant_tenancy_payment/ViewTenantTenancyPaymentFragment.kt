package softwarica.sunilprasai.cuid10748110.rental.ui.view_tenant_tenancy_payment
...
private const val TAG = "ViewTenantPaymentFragme"
...
class ViewTenantTenancyPaymentFragment : Fragment(), SwipeRefreshLayout.OnRefreshListener {
        ...
        mViewModel.getTenantTenancies().observe(requireActivity()) {
            if (isAdded) {
                if (it != null && mParamTenancyPosition != -1 && mParamPaymentPosition != -1 && mParamPaymentPosition < it[mParamTenancyPosition].payments!!.size && it[mParamTenancyPosition].id == mParamTenancyID && it[mParamTenancyPosition].payments!![mParamPaymentPosition].id == mParamPaymentID) {
                    mTenancy = mViewModel.getTenantTenancy(mParamTenancyPosition)
                    mPayment = mViewModel.getTenantTenancyPayment(
    ...
    companion object {
        ...
        fun newInstance(
            ...
            ViewTenantTenancyPaymentFragment().apply {
    ...
    override fun onRefresh() {
        ...
        tenantResponse.enqueue(object : Callback<ResponseBody> {
            override fun onResponse(
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    if (errorResponse.has("message")) {
                        ...
                        displayAPIErrorResponseDialog("Unable to refresh your payment details. Please try again later")
                    }
                } else {
                    displayAPIErrorResponseDialog("Unable to refresh your payment details. Please try again later")
                }
